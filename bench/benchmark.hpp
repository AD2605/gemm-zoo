...
void benchmark_kernel(int m, int n, int k, TOut alpha, TOut beta,
                      const int num_repitions, sycl::queue& queue) {
  using InputSharedPtr = std::shared_ptr<TIn>;
  using OutputSharedPtr = std::shared_ptr<TOut>;
  auto memory_deleter = [queue](TIn* ptr) { sycl::free(ptr, queue); };
  ...
  const std::size_t min_repitions_required =
      std::max(static_cast<std::size_t>(num_repitions), num_problems_required);
  ...
  output_d_pointer =
      OutputSharedPtr(sycl::malloc_device<TOut>(m * n, queue), memory_deleter);
  for (std::size_t i = 0; i < min_repitions_required; i++) {
    a_pointers.emplace_back(std::move(InputSharedPtr(
        sycl::malloc_device<TIn>(m * k, queue), memory_deleter)));
    b_pointers.emplace_back(std::move(InputSharedPtr(
        sycl::malloc_device<TIn>(k * n, queue), memory_deleter)));
    c_pointers.emplace_back(std::move(OutputSharedPtr(
        sycl::malloc_device<TOut>(m * n, queue), memory_deleter)));
  }
  ...
  const std::size_t flops_per_problem =
      static_cast<std::size_t>(m) * n * (2 * k + 2);
  ...
  std::cout << "Achieved GBPS: " << achieved_gbps
            << " Achieved GFLOPS: " << achieved_flops << std::endl;
}
}  // namespace detail

template <typename Kernel, typename TIn, typename TOut>
void benchmark_sizes(const std::vector<std::tuple<int, int, int>>& sizes,
                     sycl::queue& queue, const int num_repitions = 50) {
