#ifndef NVIDIA_KERNEL_FUNCTORS_ASYNC_BUFFERED_GEMM_CUH
#define NVIDIA_KERNEL_FUNCTORS_ASYNC_BUFFERED_GEMM_CUH

#include "defines.hpp"
#include "kernels/sm_80/async_buffered_gemm.cuh"
...
template <typename TIn, typename TOut, int M, int N, int K, int TM, int TN,
          int TK>
struct async_buffered_gemm {
  async_buffered_gemm(std::size_t m, std::size_t n, std::size_t k,
    ...
    checkCudaError(cudaFuncSetAttribute(
        nvidia::kernels::sm80::async_buffered_gemm<TIn, TOut, M, N, K, TM, TN,
                                                   TK, 256, 3>,
  ...
  void operator()(const TIn* a, const TIn* b, const TOut* c, TOut* d,
    ...
    nvidia::kernels::sm80::async_buffered_gemm<TIn, TOut, M, N, K, TM, TN, TK,
                                               256, 3>
